#!/bin/bash
# Launch E22 expert/multi-head diagnostics after E21.
...
REPO_ON_NIBI="$WEEKEND_ROOT/repo_e22_expert_multihead"
SOURCE_VARIANT_STAMP="20260514T002301Z"
CACHE_STAMP="20260514T002301Z"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_NAME="E22diag_submit"
...
GPU_GRES="gpu:h100:1"
GPU_EXCLUDE="g19"
WANDB_GROUP="weekend-20260502-e22-expert-multihead"
DRY_RUN="false"
...
usage() {
  cat <<'USAGE'
Usage:
  bash drac/scripts/submit_multispecies_e22_expert_multihead_ladder.sh [options]

E22 reuses the E16 combined MAT cache and launches per-species experts,
three/four-species per-species-head models, posthoc ensembles, and report/audit
collection.
...
PIPELINE_DIR="$WEEKEND_ROOT/pipeline_runs/e22_expert_multihead_diagnostics_${STAMP}"
LOG_DIR="$PIPELINE_DIR/logs"
VARIANT_ROOT="$WEEKEND_ROOT/manifests/e22_expert_multihead_${STAMP}"
...
echo "Started E22 expert/multi-head diagnostics at $(date -Is)"
echo "Host: $(hostname)"
...
SUBMITTED_TSV="$PIPELINE_DIR/e22_training_submitted.tsv"
PLAN_TSV="$PIPELINE_DIR/e22_training_plan.tsv"
REPORT_DIR="$PIPELINE_DIR/e22_report"
...
if [[ "$existing_count" -lt "$expected_count" ]]; then
  echo "Extracted cache is incomplete; refusing to submit E22 until cache is repaired." >&2
  exit 2
fi

echo "Running E22 smoke checks"
python - <<'PY'
import torch
...
PY
python -u scripts/analysis/e22_collect_expert_multihead_report.py \
  --smoke-test \
  --output-dir "$PIPELINE_DIR/e22_report_smoke"

echo "Building E22 diagnostic variants"
python -u scripts/data/multilabel/build_e22_expert_multihead_variants.py \
...
train_cmd=(
  ...
  --wandb-group "$WANDB_GROUP"
  --wandb-name "$run_exp"
  --wandb-tags "multilabel,species,E22,expert,multihead,$variant_name,$encoder,$fusion,$head_type,$loss_mode,$bands_arg,$band_availability,$class_band_mask,$crop_mode,crop${crop_seconds}"
)
...
submit_report_job() {
  ...
  local report_script="$LOG_DIR/e22_collect_report.sbatch"
  cat > "$report_script" <<REPORT_EOF
#!/bin/bash
#SBATCH --job-name=E22collect_report
...
python -u scripts/analysis/e22_collect_expert_multihead_report.py \\
  ...
  --max-rows-per-kind 12
REPORT_EOF
  local report_job
  report_job="$(sbatch --dependency=afterany:$dep "$report_script" | awk '{print $4}')"
  echo -e "$report_job\tE22_collect_report\t$REPORT_DIR\t$report_script" >> "$PIPELINE_DIR/e22_report_submitted.tsv"
  echo "Submitted E22 report collector as $report_job after jobs: $dep"
}

SHAPES_10S="low:391x50,mid:256x100,high:256x312"

submit_train "E22_fin_whale_low_expert" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low" "species:Bp" "ONC" "ONC" "low"
submit_train "E22_blue_whale_low_expert" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low" "species:Bm" "ONC" "ONC" "low"
submit_train "E22_humpback_whale_lowmid_expert" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid" "species:Mn" "ONC" "ONC" "mid"
submit_train "E22_killer_whale_onc_only_midhigh_expert" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
submit_train "E22_killer_whale_dclde_only_midhigh_expert" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "DCLDE" "DCLDE" "high"
submit_train "E22_killer_whale_onc_dclde_midhigh_sourcecap" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
submit_train "E22_three_species_multihead_lowmid" "balanced" "resnet18" "gated" "per_species" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid" "species:Bp,species:Bm,species:Mn" "ONC" "ONC" "mid"
submit_train "E22_three_species_multihead_lowmid_sourcecap" "balanced" "resnet18" "gated" "per_species" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid" "species:Bp,species:Bm,species:Mn" "ONC" "ONC" "mid"
submit_train "E22_three_species_multihead_lowmid_labelcap" "balanced" "resnet18" "gated" "per_species" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid" "species:Bp,species:Bm,species:Mn" "ONC" "ONC" "mid"
submit_train "E22_four_species_multihead_allbands_sourcecap" "balanced" "resnet18" "gated" "per_species" "0.0003" "false" "balanced_bce" "source_or_metadata" "odont_high" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid,high" "species:Bp,species:Bm,species:Mn,species:Oo" "ONC" "ONC" "high"
...
echo "Submitted jobs:"
cat "$SUBMITTED_TSV"
if [[ -f "$PIPELINE_DIR/e22_report_submitted.tsv" ]]; then
  echo "Report collector:"
  cat "$PIPELINE_DIR/e22_report_submitted.tsv"
fi
echo "Finished E22 expert/multi-head submission at $(date -Is)"
