#!/bin/bash
# Launch E28 full-source one-vs-rest per-species expert ablation.
...
WANDB_GROUP="weekend-20260502-e28-full-one-vs-rest"
DRY_RUN="false"
...
usage() {
  cat <<'USAGE'
Usage:
  bash drac/scripts/submit_multispecies_e28_full_one_vs_rest_experts.sh [options]

E28 repeats E27 one-vs-rest training, but includes ONC, BioDCASE, and DCLDE
rows. It submits one 3-hour MIG slice per species by default, followed by an
...
PIPELINE_DIR="$WEEKEND_ROOT/pipeline_runs/e28_full_one_vs_rest_experts_${STAMP}"
LOG_DIR="$PIPELINE_DIR/logs"
VARIANT_ROOT="$WEEKEND_ROOT/manifests/e28_full_one_vs_rest_experts_${STAMP}"
...
SUBMITTED_TSV="$PIPELINE_DIR/e28_training_submitted.tsv"
PLAN_TSV="$PIPELINE_DIR/e28_training_plan.tsv"
REPORT_DIR="$PIPELINE_DIR/e28_report"
...
echo "Started E28 full-source one-vs-rest expert ablation at $(date -Is)"
echo "Repo: $REPO"
...
if [[ "$existing_count" -lt "$expected_count" ]]; then
  echo "Extracted cache is incomplete; refusing to submit E28 until cache is repaired." >&2
  exit 2
fi

echo "Running E28 smoke checks"
python -u scripts/analysis/e28_collect_full_one_vs_rest_report.py \
  --submitted-tsv "$PIPELINE_DIR/does_not_exist.tsv" \
  --plan-tsv "$PIPELINE_DIR/does_not_exist.tsv" \
  --output-dir "$PIPELINE_DIR/e28_report_smoke"
python -u scripts/train/train_multiband_multilabel.py --help >/dev/null

echo "Building E28 full-source one-vs-rest variants"
python -u scripts/data/multilabel/build_e28_full_one_vs_rest_expert_variants.py \
...
set -euo pipefail
echo "Starting $run_exp stage \${E28_STAGE:-1} at \$(date -Is)"
cd "$REPO_ON_NIBI"
source .venv/bin/activate
export PYTHONPATH="\$PWD:\${PYTHONPATH:-}"
mkdir -p "$train_dir"
...
META
echo "Finished $run_exp stage \${E28_STAGE:-1} at \$(date -Is)"
TRAIN_EOF
  ...
  for stage in $(seq 1 "$CHAIN_DEPTH"); do
    local sbatch_args=(--export=ALL,E28_STAGE="$stage")
...
submit_report_job() {
  ...
  local report_script="$LOG_DIR/e28_collect_report.sbatch"
  cat > "$report_script" <<REPORT_EOF
#!/bin/bash
#SBATCH --job-name=E28collect_report
...
REPORT_EOF
  ...
  echo -e "$report_job\tE28_collect_report\t$REPORT_DIR\t$report_script" >> "$PIPELINE_DIR/e28_report_submitted.tsv"
  echo "Submitted E28 report collector as $report_job after jobs: $dep"
}

SHAPES_10S="low:391x50,mid:256x100,high:256x312"

submit_chain "E28_fin_whale_low_full_one_vs_rest" "r18_lr3e4_c10_d03" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low" "species:Bp" "ONC" "ONC" "low" "0.3"
submit_chain "E28_blue_whale_low_full_one_vs_rest" "r18_lr3e4_c10_d03" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low" "species:Bm" "ONC" "ONC" "low" "0.3"
submit_chain "E28_humpback_whale_lowmid_full_one_vs_rest" "r18_lr1e4_c10_d03" "resnet18" "gated" "shared" "0.0001" "false" "balanced_bce" "all" "audit_v2" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "low,mid" "species:Mn" "ONC" "ONC" "mid" "0.3"
...
echo "Submitted jobs:"
cat "$SUBMITTED_TSV"
if [[ -f "$PIPELINE_DIR/e28_report_submitted.tsv" ]]; then
  echo "Report collector:"
  cat "$PIPELINE_DIR/e28_report_submitted.tsv"
fi
echo "Report dir: $REPORT_DIR"
echo "Finished E28 full-source one-vs-rest expert ablation submission at $(date -Is)"
