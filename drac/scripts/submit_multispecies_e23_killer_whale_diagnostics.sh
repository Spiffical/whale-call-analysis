#!/bin/bash
# Launch E23 killer whale localization/split diagnostics after E22.
...
REPO_ON_NIBI="$WEEKEND_ROOT/repo_e23_killer_whale"
SOURCE_VARIANT_STAMP="20260514T002301Z"
CACHE_STAMP="20260514T002301Z"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_NAME="E23killer_submit"
...
GPU_GRES="gpu:h100:1"
GPU_EXCLUDE="g19"
WANDB_GROUP="weekend-20260502-e23-killer-whale-diagnostics"
RAW_ONC_ANNOTATIONS="$FINAL2025_ROOT/part2/full_bundle/manifests/annotations_all.csv"
ONC_AUDIO_DIR="$FINAL2025_ROOT/part2/full_bundle/raw_audio"
DRY_RUN="false"
...
usage() {
  cat <<'USAGE'
Usage:
  bash drac/scripts/submit_multispecies_e23_killer_whale_diagnostics.sh [options]

E23 reuses the E16 combined MAT cache, preserves raw ONC killer whale
time/frequency annotation metadata, builds clip/date-held-out diagnostics, and
renders ONC killer whale review contact sheets.
...
Options:
  ...
  --raw-onc-annotations P   Default: final2025 Part 2 annotations_all.csv
  --onc-audio-dir P         Default: final2025 Part 2 raw_audio
  --epochs N                Default: 45
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --raw-onc-annotations) RAW_ONC_ANNOTATIONS="$2"; shift 2 ;;
    --onc-audio-dir) ONC_AUDIO_DIR="$2"; shift 2 ;;
...
PIPELINE_DIR="$WEEKEND_ROOT/pipeline_runs/e23_killer_whale_diagnostics_${STAMP}"
LOG_DIR="$PIPELINE_DIR/logs"
VARIANT_ROOT="$WEEKEND_ROOT/manifests/e23_killer_whale_diagnostics_${STAMP}"
...
if [[ "$AS_COORDINATOR" != "true" ]]; then
  ...
  submit_args=(
    ...
    --stamp "$STAMP"
    --run-name "$RUN_NAME"
    --raw-onc-annotations "$RAW_ONC_ANNOTATIONS"
    --onc-audio-dir "$ONC_AUDIO_DIR"
    --epochs "$EPOCHS"
...
echo "Started E23 killer whale diagnostics at $(date -Is)"
echo "Host: $(hostname)"
...
SUBMITTED_TSV="$PIPELINE_DIR/e23_training_submitted.tsv"
PLAN_TSV="$PIPELINE_DIR/e23_training_plan.tsv"
AUDIT_DIR="$PIPELINE_DIR/onc_killer_whale_annotation_audit"
REPORT_DIR="$PIPELINE_DIR/e23_report"
...
mkdir -p "$PIPELINE_DIR" "$LOG_DIR" "$VARIANT_ROOT" "$AUDIT_DIR" "$REPORT_DIR"
cd "$REPO"
...
echo "Raw ONC annotations: $RAW_ONC_ANNOTATIONS"
echo "ONC audio dir: $ONC_AUDIO_DIR"
...
if [[ ! -f "$RAW_ONC_ANNOTATIONS" ]]; then
  echo "Missing raw ONC annotations: $RAW_ONC_ANNOTATIONS" >&2
  exit 2
fi
...
if [[ "$existing_count" -lt "$expected_count" ]]; then
  echo "Extracted cache is incomplete; refusing to submit E23 until cache is repaired." >&2
  exit 2
fi

echo "Running E23 smoke checks"
python - <<'PY'
import torch
...
out = model({"mid": torch.randn(2, 1, 16, 12), "high": torch.randn(2, 1, 16, 12)})
assert tuple(out.shape) == (2, 1), tuple(out.shape)
PY
python -u scripts/analysis/audit_onc_killer_whale_annotations.py \
  --annotations-csv "$RAW_ONC_ANNOTATIONS" \
  --manifest-csv "$INPUT_VARIANT/standardized_manifest.csv" \
  --output-dir "$PIPELINE_DIR/e23_audit_smoke" \
  --max-annotations 2 \
  --skip-images
python -u scripts/analysis/e23_collect_killer_whale_report.py \
  --submitted-tsv "$PIPELINE_DIR/does_not_exist.tsv" \
  --output-dir "$PIPELINE_DIR/e23_report_smoke"

echo "Building E23 killer whale diagnostic variants"
python -u scripts/data/multilabel/build_e23_killer_whale_diagnostics.py \
...
echo "Rendering ONC killer whale annotation audit"
audit_args=(
  python -u scripts/analysis/audit_onc_killer_whale_annotations.py
  --annotations-csv "$RAW_ONC_ANNOTATIONS"
  --manifest-csv "$INPUT_VARIANT/standardized_manifest.csv"
...
if [[ -d "$ONC_AUDIO_DIR" ]]; then
  audit_args+=(--raw-audio-dir "$ONC_AUDIO_DIR")
else
  echo "ONC audio directory not found; writing review CSV only: $ONC_AUDIO_DIR" >&2
  audit_args+=(--skip-images)
fi
if ! "${audit_args[@]}"; then
  echo "Image audit failed; retrying review CSV without image rendering" >&2
  python -u scripts/analysis/audit_onc_killer_whale_annotations.py \
    --annotations-csv "$RAW_ONC_ANNOTATIONS" \
    --manifest-csv "$INPUT_VARIANT/standardized_manifest.csv" \
    --output-dir "$AUDIT_DIR" \
...
echo -e "experiment\tvariant\tmanifest\tvocab\tdataset_root\tencoder\tfusion\thead_type\tlr\tweight_decay\tuse_pos_weight\tloss_mode\tband_availability\tclass_band_mask\tcrop_mode\tcrop_seconds\tband_crop_shapes\tbatch_size\tbands\teval_label_ids\tcalibration_source_kind\teval_source_kind\trun_dir\tarchive_path\twandb_group" > "$PLAN_TSV"
submitted_job_ids=()
...
train_cmd=(
  ...
  --wandb-group "$WANDB_GROUP"
  --wandb-name "$run_exp"
  --wandb-tags "multilabel,species,E23,killer_whale,split_diagnostic,$variant_name,$encoder,$fusion,$head_type,$loss_mode,$bands_arg,$band_availability,$class_band_mask,$crop_mode,crop${crop_seconds}"
)
...
META
TRAIN_EOF
  echo -e "$run_exp\t$variant_name\t$variant_dir/standardized_manifest.csv\t$variant_dir/label_vocabulary.json\t$EXTRACT_DIR\t$encoder\t$fusion\t$head_type\t$train_lr\t$WEIGHT_DECAY\t$use_pos_weight\t$loss_mode\t$band_availability\t$class_band_mask\t$crop_mode\t$crop_seconds\t$band_crop_shapes\t$batch_size\t$bands_arg\t$eval_label_ids\t$calibration_source_kind\t$eval_source_kind\t$run_dir\t$ARCHIVE_PATH\t$WANDB_GROUP" >> "$PLAN_TSV"
  if [[ "$DRY_RUN" == "true" ]]; then
...
submit_report_job() {
  ...
  local report_script="$LOG_DIR/e23_collect_report.sbatch"
  cat > "$report_script" <<REPORT_EOF
#!/bin/bash
#SBATCH --job-name=E23collect_report
...
REPORT_EOF
  ...
  echo -e "$report_job\tE23_collect_report\t$REPORT_DIR\t$report_script" >> "$PIPELINE_DIR/e23_report_submitted.tsv"
  echo "Submitted E23 report collector as $report_job after jobs: $dep"
}

SHAPES_10S="mid:256x100,high:256x312"

submit_train "E23_killer_whale_onc_only_midhigh_clip_split" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
submit_train "E23_killer_whale_onc_only_midhigh_date_split" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
submit_train "E23_killer_whale_onc_dclde_midhigh_sourcecap_clip_split" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
submit_train "E23_killer_whale_onc_dclde_midhigh_sourcecap_date_split" "balanced" "resnet18" "gated" "shared" "0.0003" "false" "balanced_bce" "source_or_metadata" "none" "centered_gaussian" "10" "$SHAPES_10S" "$BATCH_SIZE" "mid,high" "species:Oo" "ONC" "ONC" "high"
...
echo "Submitted jobs:"
cat "$SUBMITTED_TSV"
if [[ -f "$PIPELINE_DIR/e23_report_submitted.tsv" ]]; then
  echo "Report collector:"
  cat "$PIPELINE_DIR/e23_report_submitted.tsv"
fi
echo "Audit dir: $AUDIT_DIR"
echo "Report dir: $REPORT_DIR"
echo "Finished E23 killer whale diagnostics at $(date -Is)"
