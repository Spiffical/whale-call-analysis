#!/usr/bin/env python3
"""Collect E26 ONC-only expert ablation metrics and ensemble rankings."""
...
def markdown_report(
    *,
    ...
    lines = [
        "# E26 ONC-Only Expert Ablation Report",
        "",
        "E26 repeats the winning E24 per-species expert setup but removes external BioDCASE and DCLDE training annotations. Metrics are calibrated on ONC validation rows and evaluated on ONC test rows.",
        "",
        "## Best Ensembles",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"Individual metrics CSV: `{output_dir / 'e26_individual_metrics.csv'}`",
            f"Ensemble rankings CSV: `{output_dir / 'e26_ensemble_rankings.csv'}`",
            f"Variant CSV: `{output_dir / 'e26_variant_summary.csv'}`",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    e24.write_csv(args.output_dir / "e26_individual_metrics.csv", individual)
    e24.write_csv(args.output_dir / "e26_ensemble_rankings.csv", ensembles)
    e24.write_csv(args.output_dir / "e26_variant_summary.csv", variants)
    report = markdown_report(individual=individual, ensembles=ensembles, variants=variants, output_dir=args.output_dir)
    report_path = args.output_dir / "e26_onc_only_report.md"
