#!/usr/bin/env python3
"""Collect E28 full-source one-vs-rest expert metrics and ensemble rankings."""
...
def markdown_report(
    *,
    ...
    lines = [
        "# E28 Full-Source One-vs-Rest Expert Report",
        "",
        "E28 repeats the E27 one-vs-rest expert design, but trains with ONC, BioDCASE, and DCLDE rows. Non-active labels are retained as explicit negatives. Metrics are calibrated on ONC validation rows and evaluated on ONC test rows.",
        "",
        "## Best Ensembles",
        "",
    ...
    lines.extend(
        [
            ...
            "## Comparison Targets",
            "",
            "- E24 best ensemble with external annotations, filtered expert labels: macro F1 0.9442, micro F1 0.9691, precision 0.9594, recall 0.9790, hard-negative FP 0.1846.",
            "- E27 is the ONC-only one-vs-rest ablation; compare E28 against E27 to isolate the value of external source rows under the one-vs-rest design.",
            "",
            f"Individual metrics CSV: `{output_dir / 'e28_individual_metrics.csv'}`",
            f"Ensemble rankings CSV: `{output_dir / 'e28_ensemble_rankings.csv'}`",
            f"Selected example CSVs: `{output_dir / 'ensembles'}/<ensemble>/selected_examples.csv` for retained ensembles",
            f"Variant CSV: `{output_dir / 'e28_variant_summary.csv'}`",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    e24.write_csv(args.output_dir / "e28_individual_metrics.csv", individual)
    e24.write_csv(args.output_dir / "e28_ensemble_rankings.csv", ensembles)
    e24.write_csv(args.output_dir / "e28_variant_summary.csv", variants)
    report = markdown_report(individual=individual, ensembles=ensembles, variants=variants, output_dir=args.output_dir)
    report_path = args.output_dir / "e28_full_one_vs_rest_report.md"
